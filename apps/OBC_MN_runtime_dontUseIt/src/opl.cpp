...
statusErrDef allocArrays(void)
{
    statusErrDef res = noError;

    FILE* file = fopen(NB_NODES_TXT_PATH, "r");
    if (file == NULL)
    {
        perror("Erreur lors de l'ouverture du fichier nbNodes.txt");
    ...
    NB_NODES = atoi(ligne);
    printf("ligne[0] : %d\n", NB_NODES);
    SIZE_OUT = NB_NODES * 25;
    SIZE_IN = NB_NODES * 13;
    nbValuesCN_Out = SIZE_OUT / NB_NODES - 2;
    nbValuesCN_In = SIZE_IN / NB_NODES - 1;

    values_Out_MN_l = (int16_t*)malloc(SIZE_OUT * sizeof(int16_t));
    values_In_MN_l = (int16_t*)malloc(SIZE_IN * sizeof(int16_t));

    int sizeActivated = SIZE_OUT + 2;
    activated_In_MN_l = (bool*)malloc(sizeActivated * sizeof(bool));
    if (activated_In_MN_l == NULL) {
        // Handle memory allocation failure
        fprintf(stderr, "Memory allocation failed for activated_In_MN_l.\n");
        return noError;
    }

    // Initialize each element to false
    memset(activated_In_MN_l, false, sizeActivated * sizeof(bool));

    /*for (int i = 0; i < test; i++) {
        printf("activated_In_MN_l[%d] : %d\n",i, activated_In_MN_l[i]);
    }*/
...
statusErrDef initProcessImage(void)
{
    ...
    size_t sizeOfInArray = SIZE_IN * sizeof(int16_t);
    size_t sizeOfOutArray = SIZE_OUT * sizeof(int16_t);

    
    printf("Initializing process image...\n");
    printf("Size of process image: Input = %lu Output = %lu\n",
        (ULONG)sizeOfInArray,
        (ULONG)sizeOfOutArray);
    eventlog_printMessage(kEventlogLevelInfo,
        kEventlogCategoryGeneric,
    ...
    ret = oplk_allocProcessImage(sizeOfInArray, sizeOfOutArray);
    if (ret != kErrorOk)
    ...
    pProcessImageOut_l->out_MN_array = (int16_t*)malloc(SIZE_OUT * sizeof(int16_t));
    if (pProcessImageOut_l->out_MN_array == NULL) {
        // Handle memory allocation failure for out_MN_array
        exit(EXIT_FAILURE);
    }

    pProcessImageIn_l->in_MN_array = (int16_t*)malloc(SIZE_IN * sizeof(int16_t));
    if (pProcessImageIn_l->in_MN_array == NULL) {
        // Handle memory allocation failure for in_MN_array
        free(pProcessImageOut_l->out_MN_array);
        exit(EXIT_FAILURE);
    }

    // Initialize the arrays if needed
    memset(pProcessImageOut_l->out_MN_array, 0, SIZE_OUT * sizeof(int16_t));
    memset(pProcessImageIn_l->in_MN_array, 0, SIZE_IN * sizeof(int16_t));
...
statusErrDef initProcessImage(void) {
    ...
    // Allocate memory for PI_OUT structure
    pProcessImageOut_l = (PI_OUT*)malloc(sizeof(PI_OUT));
    if (pProcessImageOut_l == NULL) {
        fprintf(stderr, "Memory allocation failed for pProcessImageOut_l.\n");
        exit(EXIT_FAILURE);
    }
    pProcessImageOut_l->out_MN_array = (int16_t*)malloc(SIZE_OUT * sizeof(int16_t));
    if (pProcessImageOut_l->out_MN_array == NULL) {
        fprintf(stderr, "Memory allocation failed for out_MN_array.\n");
        free(pProcessImageOut_l);
        exit(EXIT_FAILURE);
    }

    // Allocate memory for PI_IN structure
    pProcessImageIn_l = (PI_IN*)malloc(sizeof(PI_IN));
    if (pProcessImageIn_l == NULL) {
        fprintf(stderr, "Memory allocation failed for pProcessImageIn_l.\n");
        free(pProcessImageOut_l->out_MN_array);
        free(pProcessImageOut_l);
        exit(EXIT_FAILURE);
    }
    pProcessImageIn_l->in_MN_array = (int16_t*)malloc(SIZE_IN * sizeof(int16_t));
    if (pProcessImageIn_l->in_MN_array == NULL) {
        fprintf(stderr, "Memory allocation failed for in_MN_array.\n");
        free(pProcessImageIn_l);
        free(pProcessImageOut_l->out_MN_array);
        free(pProcessImageOut_l);
        exit(EXIT_FAILURE);
    }
    ...
    errorIndex = obdpi_setupProcessImage();
    if (errorIndex != 0) {
        eventlog_printMessage(kEventlogLevelFatal, kEventlogCategoryControl, "Setup process image failed at index 0x%04x\n", errorIndex);
        return errSetupProcessImage;
    }
...
{
    tOplkError  ret;
    //free(values_Out_MN_l);
    //free(values_In_MN_l);
    //free(activated_In_MN_l);
